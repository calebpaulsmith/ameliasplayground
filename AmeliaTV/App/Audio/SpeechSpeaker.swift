...
/// later, with this as the fallback. Bright, kid-friendly pitch. Works on both
/// tvOS and iOS. No network — keeps the privacy constraint intact.
final class SpeechSpeaker: LineSpeaker {
    private let synth = AVSpeechSynthesizer()

    /// When false, narration is silent — the game is designed to read from the
    /// graphics/HUD alone, so voice is an optional aid.
    var isEnabled = true
